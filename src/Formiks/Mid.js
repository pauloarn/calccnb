...
export default function Mid (props) {
    ...
    async function AddGame(values){
        ...
        total += (values.multiKill * 2)
        total += (values.killingSpree * 1.5)
        total += (winAux)
        ...
        const game = {
            ...
            multiKill: (values.multiKill * 2),
            killingSpree: (values.killingSpree * 1.5),
            win: winAux,
            total: total
        };
        console.log(game);
    ...
    return (
        <Formik
            initialValues = {{champ: '', abates: '', mortes: '', assists: '', gametime: '', towers:'', farm:'', champDamage:'', goalsDamage:'',multiKill: '', killingSpree: ''}}
            validationSchema = {
                yup.object().shape({
                    ...
                    multiKill: yup.number().required('Informe seu maior MultiKill'),
                    killingSpree: yup.number().required('Informe seu mairo Killing Spree'),
                })
            }
            onSubmit={(values) => {
                console.log(values)
        ...
        >
            {({values, handleChange, errors,handleSubmit})=>(                        
                <View style={styles.insideScroll}>
                    ...
                    <InputComponent
                        field = "Killing Spree"
                        onChangeText = {handleChange('killingSpree')}
                        keyboard = 'numeric'
                        error = {errors.killingSpree && errors.killingSpree}
                    />
                    <InputComponent
                        field = "Multi Kill"
                        onChangeText = {handleChange('multiKill')}
                        keyboard = 'numeric'
                        error = {errors.multiKill && errors.multiKill}
                    />     
